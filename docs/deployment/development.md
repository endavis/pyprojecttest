...
# Run with arguments
uv run python -m pyprojecttest --verbose
```
...
3. Set **Module name**: `pyprojecttest`
...
```bash
# Using pdb
uv run python -m pdb -m pyprojecttest
...
# Verify installation
uv run python -c "import pyprojecttest; print(pyprojecttest.__version__)"
```
...
uv run uvicorn pyprojecttest.app:app --reload

# Using Flask debug mode
FLASK_DEBUG=1 uv run flask run

# Using watchfiles for custom scripts
uv run watchfiles "python -m pyprojecttest" src/
```
...
```bash
# Using cProfile
uv run python -m cProfile -o profile.stats -m pyprojecttest
...
uv pip install --index-url https://test.pypi.org/simple/ pyprojecttest

# If your package has dependencies from real PyPI, use both indexes
uv pip install --index-url https://test.pypi.org/simple/ --extra-index-url https://pypi.org/simple/ pyprojecttest
```
...
# https://test.pypi.org/project/pyprojecttest/

# Verify installation works
uv venv /tmp/test-install
uv pip install --python /tmp/test-install --index-url https://test.pypi.org/simple/ pyprojecttest
```
